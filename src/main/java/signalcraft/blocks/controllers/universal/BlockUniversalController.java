...
public class BlockUniversalController extends BlockController {

    public BlockUniversalController(String name) {
        super(name);
    ...
    public void onNeighborBlockChange(final World world, final int x, final int y, final int z, final Block theBlock) {
        ...
        if (tileE instanceof TileControllerUniversal) {
            ((TileControllerUniversal) tileE).activateReceivers(world.isBlockIndirectlyGettingPowered(x, y, z));
        }
    }
    ...
    public TileEntity createNewTileEntity(World world, int i) {
        return new TileControllerUniversal();
    }
