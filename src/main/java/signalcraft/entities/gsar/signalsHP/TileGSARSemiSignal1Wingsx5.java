...
public class TileGSARSemiSignal1Wingsx5 extends TileGSARSemiSignal
{
     ...
     * [8] - Arm 2 Rotation
     */
    public TileGSARSemiSignal1Wingsx5() {
        this.setMostRestrictiveState(SignalState.STUJ);
        this.ValidStates = getValidStatesForTile();
    }
    @Override
    public void updateEntity() {
        if (this.getState().equals(SignalState.VOLNO) && this.getArm1Rotation() < 45) {
            this.setArm1Rotation(this.getArm1Rotation() + 1);
        } else if (!this.getState().equals(SignalState.VOLNO) && this.getArm1Rotation() > 0) {
            this.setArm1Rotation(this.getArm1Rotation() - 1);
        }
    }
    @Override
