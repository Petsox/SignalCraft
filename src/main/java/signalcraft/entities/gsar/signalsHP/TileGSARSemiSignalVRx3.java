...
public class TileGSARSemiSignalVRx3 extends TileGSARSemiSignal
{
    ...
    public void updateEntity() {
        if (this.getState().equals(SignalState.VOLNO) || this.getState().equals(SignalState.R40VOLNO)) {
            if (this.getState().equals(SignalState.VOLNO) && this.getArm1Rotation() < 45 && !this.getState().equals(SignalState.R40VOLNO) && this.getArm2Rotation() <= 45) {
                this.setArm1Rotation(this.getArm1Rotation() + 1);
            }
            else if (this.getState().equals(SignalState.R40VOLNO) && this.getArm1Rotation() < 45 && this.getArm2Rotation() < 45) {
                this.setArm2Rotation(this.getArm2Rotation() + 1);
            }
        }
        else if ((!this.getState().equals(SignalState.VOLNO) && this.getArm1Rotation() > 0) || (!this.getState().equals(SignalState.R40VOLNO) && this.getArm2Rotation() > 0)) {
            if (this.getArm2Rotation() > 0) {
                this.setArm2Rotation(this.getArm2Rotation() - 1);
            }
            else if (this.getArm1Rotation() > 0 && this.getArm2Rotation() < 45) {
                this.setArm1Rotation(this.getArm1Rotation() - 1);
            }
        }
    }
