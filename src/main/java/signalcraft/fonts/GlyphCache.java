...
public class GlyphCache
{
    ...
    /** If true, then enable antialiasing when rendering the font glyph */
    ...
    void setDefaultFont(String name, int size, boolean antiAlias)
    {
        ...
        try {
            final InputStream input = Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation(SignalCraft.MOD_ID +  ":fonts/" + name + ".ttf")).getInputStream();
            usedFonts.add(Font.createFont(Font.TRUETYPE_FONT, input));
        } catch (Exception e) {
    ...
    private void updateTexture(Rectangle dirty)
    {
        ...
        if(dirty != null)
        {
            ...
            ByteBuffer byteBuffer = ByteBuffer.allocateDirect(imageBuffer.capacity() * 4);
            byteBuffer.order(ByteOrder.BIG_ENDIAN); // Match imageBuffer's byte order
            for (int i = 0; i < imageBuffer.capacity(); i++) {
                int color = imageBuffer.get(i);
                // Directly pack 32-bit RGBA value into bytes without shifting
                byteBuffer.putInt(color);
            }
            byteBuffer.flip();
            GLU.gluBuild2DMipmaps(GL11.GL_TEXTURE_2D, 4, TEXTURE_WIDTH, TEXTURE_HEIGHT,
